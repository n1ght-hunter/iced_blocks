...
- `shader(&controller)` — custom `iced::advanced::Widget` (no `iced::widget::shader::Shader` wrapper) that draws the latest Servo frame and forwards iced events to Servo.
...
- `examples/basic.rs` — minimal tabbed browser: URL bar, back/forward/reload, close/new-tab, and `target="_blank"` links opening new tabs.
